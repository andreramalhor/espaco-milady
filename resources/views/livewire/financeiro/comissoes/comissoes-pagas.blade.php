            ...
            <table class="table table-sm">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @php($total_comissoes = $total_comissoes + $colaborador->opmnhtrvansmesd->where('status', '=', 'Pago')->sum('valor'))
                        <td class="text-right">{{ number_format($colaborador->opmnhtrvansmesd->where('status', '=', 'Pago')->sum('valor'), 2, ',', '.') }}</td>
                        <td class="text-center">
                            @can('###############')
                            <a href="{{ route('fin.comissoes.pagas', $colaborador->id) }}" class="text-muted" data-bs-tooltip="tooltip" data-bs-title="Ver" data-original-title="Ver"><i class="fa-solid fa-search"></i></a>
                            @endcan
                        </td>
                    </tr>
                    @endforeach
