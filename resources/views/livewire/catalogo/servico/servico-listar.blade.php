<div>
    <div class="card">
        ...
        <div class="card-body table-responsive p-0">
            <div class="row p-2">
                <div class="col-md-2">
                    <a class="btn btn-secondary btn-block btn-sm float-right" href="{{ route('cat.servicos.criar') }}"><i class="fa fa-plus"></i> Cadastrar novo serviço</a>
                </div>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    @forelse ($servicos as $ciclo)
                        <tr wire:key="{{ $ciclo->id }}" class="{{ is_null($ciclo->deleted_at) ? '' : 'table-danger' }}">
                            ...
                            <td class="p-1 text-right">
                                <a class="btn btn-sm" href="{{ route('cat.servicos.comissoes', $ciclo->id) }}">Comissões</a>
                                <div class="btn-group">
                                    <a href="#" data-bs-toggle="dropdown"><span class="badge bg-info dropdown-toggle dropdown-icon">Ações  </span></a>
                                    <div class="dropdown-menu">
                                        <a class="dropdown-item" href="{{ route('cat.servicos.mostrar', $ciclo->id) }}">Sobre</a>
                                        <a class="dropdown-item" href="{{ route('cat.servicos.editar', $ciclo->id) }}">Editar</a>
                                        <a class="dropdown-item" href="{{ route('cat.servicos.comissoes', $ciclo->id) }}">Comissões</a>
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="text-center"><small>Não há serviços cadastradas</small></td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
    ...
    <!-- push('scripts') -->
        <script>
            window.addEventListener('servicoDeleted', event => {
                console.log(event)
            });
