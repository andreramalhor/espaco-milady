<div class="row">
	...
	<div class="col-8">
		<div class="card {{ $etapa == 'etapa-1' ? 'card-secondary' : 'collapsed-card' }}">
			...
			<div class="card-header">
				...
				<div class="card-tools">
					@if( is_object($cliente) && $cliente->saldo_conta > 0 )
					<button type="button" class="btn btn-success btn-sm" wire:click="modal_saldo">
						<i class="fas fa-circle-dollar-to-slot"></i>
						<span class="badge bg-default">R$ {{ number_format($cliente->saldo_conta, 2, ',', '.') }}</span>
					</button>
					@elseif( is_object($cliente) && $cliente->saldo_conta < 0 )
					...
					@endif
					@if(is_object($cliente) && $cliente->agendamentos_hoje->count() > 0)
					<button type="button" class="btn btn-warning btn-sm" wire:click="modal_agendamentos">
						<i class="far fa-calendar-alt"></i>
						<span class="badge bg-default text-dark">{{ $cliente->agendamentos_hoje->count() }}</span>
					</button>
					@else
					<button type="button" class="btn btn-default btn-sm disabled">
						<i class="far fa-calendar-alt"></i>
					...
					@endif
					<button type="button" class="btn btn-tool" data-card-widget="collapse" wire:click="mudar_etapa('etapa-1')">{!! $etapa == 'etapa-1' ? '<i class="fas fa-minus"></i>' : '<i class="fas fa-plus"></i>' !!}</button>
				</div>
				<br>
				<div class="float-left">
					{{ optional($cliente)->apelido ?? '' }}
				</div>
			</div>
			<div class="card-body" style="display: {{ $etapa == 'etapa-1' ? 'block' : 'none' }};">
				<div class="row">
					<div class="col-1">
						<div class="form-group">
							<label for="col-form-label">#</label>
							<input type="text" class="form-control form-control-sm" value="{{ $cliente->id ?? '' }}" readonly>
						</div>
					</div>
					<div class="col-6">
						<div class="form-group">
							...
							<select class="form-control form-control-sm" wire:change="cliente_selecionado( $event.target.value )">
								...
								@foreach(\App\Models\Atendimento\Pessoa::orderBy('nome', 'asc')->get() as $opt_cliente)
								<option value="{{ $opt_cliente->id }}">{{ $opt_cliente->nomes }}</option>
								@endforeach
							</select>
						</div>
					</div>
			...
			<div class="card-footer">
				...
				@if( $etapa != 'etapa-1' )
				<button class="btn btn-secondary float-right" wire:click="mudar_etapa( 'etapa-2' )">Próximo</button>
				@endif
			</div>	
	...
	@if(!is_null($cliente))
	<div class="col-12">
		<div class="card {{ $etapa == 'etapa-2' ? 'card-secondary' : 'collapsed-card' }}">
			<div class="overlay d-none" wire:loading.class.remove="d-none">
				<i class="fas fa-2x fa-sync-alt fa-spin"></i>
			</div>
			<div class="card-header">
				...
				<div class="card-tools">
					<button type="button" class="btn btn-tool" data-card-widget="collapse" wire:click="mudar_etapa('etapa-2')">{!! $etapa == 'etapa-2' ? '<i class="fas fa-minus"></i>' : '<i class="fas fa-plus"></i>' !!}</button>
				</div>
			</div>
			<div class="card-body p-0" style="display: {{ $etapa == 'etapa-2' ? 'block' : 'none' }};">
				<table class="table table-sm table-striped">
					<thead class="table-dark">
						<tr>
							<th class="text-center" style="width: 4%;">#</th>
							<th class="text-left" style="width: 28%;">Serviço</th>
							<th class="text-right" style="width: 8%;">Valor Und.</th>
							<th class="text-right" style="width: 8%;">Desc. ou Acrs.</th>
							<th class="text-right" style="width: 8%;">Valor final</th>
							<th class="text-left" style="width: 15%;">Profissional</th>
							<th class="text-center" style="width: 5%;">%</th>
							<th class="text-right" style="width: 8%;">Comissão</th>
					...
					<tbody>
						<tr>
							...
							<th class="text-left">
								<select class="form-control form-control-sm" wire:model.live="id_pessoa" wire:change="profissional_selecionado()" {{ ( $id_servprod == '' || count($profissionals) == 0 ) ? 'disabled="disabled"' : '' }}>
									@if($id_servprod == '')
									<option value='' selected>Selecione serviço / produto . . .</option>
									@endif
									@if( count(value: $profissionals) == 0)
									<option value='' selected>Não há profissionais</option>
									@endif
									<option value=''>Selecione . . .</option>
							...
							<th class="" style="width: 5%;">
								<button type="button" class="btn btn-block btn-success btn-sm {{ $bt_detalhe_adicionar }}" wire:click="detalhe_adicionar({ origem_adicao: '{{ $tipo_adicao }}', dados: null, etapa: 'etapa-2' })"><i class="fa-solid fa-floppy-disk"></i></button>
							</th>
						</tr>						
						...
						<tr :key="$item->id">
							...
							<td class="">
								<button type="button" class="btn btn-warning btn-sm" wire:click="detalhe_editar_selecionar({{ $item['id_chave'] }})"><i class="fa-solid fa-edit"></i></button>
								<button type="button" class="btn btn-danger btn-sm" wire:click="detalhe_remover({{ $item['id_chave'] }})"><i class="fa-solid fa-xmark"></i></button>
							</td>
						</tr>
						@endforeach
					</tbody>
			...
			<div class="card-footer">
				<button class="btn btn-default">Cancelar</button>
				<button class="btn btn-secondary float-right" wire:click="mudar_etapa( 'etapa-3' )">Próximo</button>
			</div>
		</div>
	</div>
	@endif
	@if(!is_null($cliente))
	<div class="col-12">
		<div class="card {{ $etapa == 'etapa-3' ? 'card-secondary' : 'collapsed-card' }}">
			<div class="overlay d-none" wire:loading.class.remove="d-none">
				<i class="fas fa-2x fa-sync-alt fa-spin"></i>
			</div>
			<div class="card-header">
				...
				<div class="card-tools">
					<button type="button" class="btn btn-tool" data-card-widget="collapse" wire:click="mudar_etapa('etapa-3')">{!! $etapa == 'etapa-3' ? '<i class="fas fa-minus"></i>' : '<i class="fas fa-plus"></i>' !!}</button>
				</div>
			</div>
			<div class="card-body" style="display: {{ $etapa == 'etapa-3' ? 'block' : 'none' }};">
				<div class="row">
					<div class="px-1" style="width: 20%;">
						<label clas="mb-0">Forma de Pagamento</label>
					</div>
					@if(!is_null(value: $pgto_forma) && $opcoes_bandeiras->count() > 1)
					<div class="px-1" style="width: 15%;">
						<label clas="mb-0">Bandeira</label>
					</div>
					@endif
					@if(!is_null(value: $pgto_bandeira) && $opcoes_parcelas->count() > 1)
					<div class="px-1" style="width: 10%;">
						<label clas="mb-0">Parcelas</label>
					</div>
					@endif
					@if( !is_null($forma_escolhida) && $forma_escolhida->count() == 1 && $forma_escolhida->first()->recebimento == 'Manual' )
					<div class="px-1" style="width: 15%;">
				...
				<div class="row">
					<div class="px-1" style="width: 20%;" wire:ignore>
						<select class="form-control form-control-sm" wire:model="pgto_forma" wire:change="forma_selecionando( 'forma' )">
					...
					@if(!is_null($pgto_bandeira) || (!is_null(value: $pgto_forma) && $opcoes_bandeiras->count() > 1))
					<div class="px-1" style="width: 15%;">
						<select class="form-control form-control-sm" wire:model="pgto_bandeira" wire:change="forma_selecionando( 'bandeira' )">
							<option value=''>Selecione a bandeira . . .</option>
							@foreach($opcoes_bandeiras as $bandeira)
							<option value="{{ $bandeira }}" {{ $bandeira ==  'Dinheiro' ? 'selected' : '' }}>{{ $bandeira }}</option>
							@endforeach
						</select>
					</div>
					@endif
					@if(!is_null(value: $pgto_bandeira) && $opcoes_parcelas->count() > 1)
					<div class="px-1" style="width: 10%;">
						<select class="form-control form-control-sm" wire:model="pgto_parcela" wire:change="forma_selecionando( 'parcela' )">
							<option value=''>Selecione parcela . . .</option>
					...
					<div class="px-1" style="width: 4%;">
						...
						@if(is_null($forma_escolhida) || $forma_escolhida->count() != 1)
						disabled
						@endif
						" wire:click="adicionar_pagamento()"><i class="fa-solid fa-arrow-alt-circle-down"></i></button>
						@endif
					</div>
				</div>
				<hr>
				...
				<div class="row pb-1">
					...
					<div class="px-1" style="width: 4%;">
						<button type="button" class="btn btn-block btn-danger btn-sm text-center" wire:click="pagamento_remover('{{ $pagamento['id_grupo'] }}')"><i class="fa-solid fa-trash-can"></i></button>
					</div>
				</div>
				@empty
