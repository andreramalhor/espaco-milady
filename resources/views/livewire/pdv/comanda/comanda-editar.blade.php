		...
		<div class="card">
			...
			<div class="card-body">
					...
					<div class="row pb-1">
						...
						<div class="px-1" style="width: 20%;">
							<select class="form-control form-control-sm" wire:model.live="id_pessoa" wire:change="profissional_selecionado( $event.target.value , false )">
								...
								<option {{ $id_pessoa == $profissional->id_profexec ? 'selected' : '' }} value="{{ $profissional->id_profexec }}">{{ optional($profissional->fwpokkeoewfeojd)->apelido }} | ({{ $profissional->prc_comissao * 100 }} %)</option>
								@endforeach
								@endif
							</select>
						</div>
				...
				<div class="row pb-1">
					...
					<div class="px-1" style="width: 4.5%;">
						<input type="number" class="form-control form-control-sm text-center" value="{{ number_format($detalhe->quantidade, 0) }}" readonly="readonly">
					</div>
					<div class="px-1" style="width: 6%;">
