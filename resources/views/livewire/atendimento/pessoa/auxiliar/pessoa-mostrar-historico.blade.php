<div class="row p-0">
    <table class="table table-bordered table-sm table-striped">
        ...
        <tbody>
            <tr style="border-left: 5px solid white;">
                ...
                <td class="p-0 pl-1 py-1">
                    <select class="form-control form-control-sm form-block" wire:model.change="p_prd_srv">
                        ...
                        @foreach(\App\Models\Catalogo\ServicoProduto::orderBy('nome')->get() as $prod_serv)
                        <option value="{{ $prod_serv->id }}">{{ $prod_serv->nome }}</option>
                        @endforeach
                    </select>
                </td>
                <td class="p-0"></td>
            ...
            @foreach($prod_e_serv as $detalhe)
            <tr>
                <td class="text-center">{{ \Carbon\Carbon::parse($detalhe->created_at)->format('d/m/Y H:i') }}</td>
                <td class="text-center">{{ $detalhe->id_venda }}
                    <a wire:click="$dispatch('pdv-comanda-comandamostrar', { id: {{ $detalhe->id_venda }} })" class="btn btn-xs btn-default" data-bs-tooltip="tooltip" data-bs-title="visualizar" data-original-title="visualizar"><i class="fas fa-search fa-fw"></i></a>
                </td>
                <td class="text-left">{{ $detalhe->kcvkongmlqeklsl->nome ?? $detalhe->id_servprod }}</td>
