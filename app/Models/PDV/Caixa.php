...
// RELACIONAMENTOS  ===========================================================================================
  ...
  public function oiefhueyedosaia()
  {
    return $this->hasManyThrough(
  ...
  public function qowuewoiehdslks()
  {
    return $this->hasMany(LancamentoGeral::class, 'id_caixa', 'id');    
  }

  // VERIFICAR SE ESTA CORRETO E SE ESTA UTILIZANDO
  public function fjjeiofuwerwqouw() 
  {
    return $this->hasMany(RecebimentoCartao::class, 'id_caixa', 'id');    
  }
...
// MÉTODOS          ===========================================================================================
  ...
  public function getSaldoAtualDoisAttribute()
  {
    $saldo_atual = 
                   $this->ssqlnxsbyywplan->where('id_forma_pagamento', 1)->sum('valor')
                 - $this->qowuewoiehdslks->where('id_forma_pagamento', 1)->where('tipo', 'D')->sum('vlr_final')
                 + $this->qowuewoiehdslks->where('id_forma_pagamento', 1)->where('tipo', 'R')->sum('vlr_final')
                 + $this->qowuewoiehdslks->where('id_forma_pagamento', 1)->where('tipo', 'T')->sum('vlr_final')
                 + $this->druxvxuskbnfggv->sum('vlr_final')
                 + $this->vlr_abertura;
